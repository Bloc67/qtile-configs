...
layouts["box"] = function ()
    ...
    lo = add_layout("config-save")
    lo.geometry =
        {x = posX - pos_offsetX, y = bigbtnrowY, an = 5, w = 70, h = 18}
...
function bar_layout(direction)
    ...
    -- Configuration Save
    geo = { x = geo.x - geo.w - padX - 18, y = geo.y, an = geo.an, w = geo.w, h = geo.h }
    lo = add_layout("config-save")
    lo.geometry = geo
    lo.style = osc_styles.smallButtonsBar
...
function osc_init()
    ...
    --config-save
    ne = new_element("config-save", "button")

    ne.content = "\240\159\150\171"
    ne.eventresponder["mbtn_left_up"] =
        function ()
            addShow()
        end --Logic here
...
function subSelect()
    ...
    if file then
        file:close()
        for line in io.lines(path .. "\\.conf") do 
            savedSub[#savedSub + 1] = line
        end
        mp.set_property("aid", savedSub[1])
        mp.set_property("sid", savedSub[2])
        mp.set_property("sub-pos", savedSub[3])
        mp.set_property("sub-font-size", savedSub[4])
        mp.set_property("sub-scale", savedSub[5])
        mp.set_property("sub-ass-override", savedSub[6])
    end
end
